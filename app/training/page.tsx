...
import { freeGuides, premiumGuides, type Guide } from '@/lib/guides'
...
function GuideCard({ guide }: { guide: Guide }) {
  return (
    <Link
      ...
      className={`card p-6 hover:-translate-y-1 block ${guide.premium ? 'border-2 border-brand-golden/40 relative' : ''}`}
    >
      {guide.premium ? (
        <>
          <div className="absolute top-4 right-4 badge bg-brand-golden text-plum">★ Premium</div>
          <span className="text-3xl block mb-3">{guide.emoji}</span>
          <h3 className="font-extrabold text-plum text-lg leading-snug mb-2 pr-20">{guide.title}</h3>
        </>
      ) : (
        <>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-brand-orange font-bold text-sm">{guide.premium ? 'Preview →' : 'Read guide →'}</span>
      </div>
    </Link>
  )
}
    ...
    <div className="bg-brand-cream min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-14">
          {freeGuides.map(guide => <GuideCard key={guide.slug} guide={guide} />)}
        </div>

        {/* Premium guides */}
        <div className="flex items-center gap-3 mb-2 flex-wrap">
          <h2 className="text-2xl font-extrabold text-plum">Premium Guides</h2>
          <span className="badge bg-brand-golden/20 text-plum">★ Members-only tier coming soon</span>
        </div>
        <p className="text-plum/60 text-sm mb-6 max-w-2xl">
          Our most in-depth, structured programs. The premium tier isn&apos;t open yet — join free and we&apos;ll
          email you when it launches.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-14">
          {premiumGuides.map(guide => <GuideCard key={guide.slug} guide={guide} />)}
        </div>
